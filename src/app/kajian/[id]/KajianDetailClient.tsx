                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 space-y-6">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        {true && (
                                            ...
                                            >
                                                <img
                                                    src={(() => {
                                                        const isTarawih = kajian.tema?.toLowerCase().includes('tarawih') || kajian.waktu?.toLowerCase().includes('tarawih') || kajian.waktu?.toLowerCase().includes('tarweh');
                                                        ...
                                                        // Force Tarawih cover if detected, even if DB has something else (especially if it has the wrong friday cover)
                                                        if (isTarawih) return '/images/tarawih-cover.svg';

                                                        return kajian.imageUrl || (isFriday ? '/images/khutbah-jumat-cover.png' : '/images/default-kajian.png');
                                                    })()}
                                                    alt={kajian.tema}
                                                    onError={(e) => {
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    ...
                                    {kajian.catatan && (
                                        <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-2xl">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <p className="text-xs font-black text-yellow-700 uppercase tracking-widest mb-1">Catatan Panitia</p>
                                                    <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{kajian.catatan}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        {/* Contact Persons */}
                                        {contactPersons.map((phone, idx) => (
                                            <a
                                                key={`cp-${idx}`}
                                                href={`https://wa.me/${phone.replace(/\D/g, '').replace(/^0/, '62')}`}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="flex items-center justify-center gap-2 py-3 bg-emerald-50 text-emerald-700 rounded-xl font-bold text-sm hover:bg-emerald-100 transition-colors"
                                            >
                                                ...
                                                Hubungi {idx === 0 ? 'Panitia' : `CP ${idx + 1}`}
                                            </a>
                                        ))}
